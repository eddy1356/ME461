...
//			my_scanf(rxbuff,&var1,&var2,&var3,&var4);
			my_scanf(rxbuff,&temp, &temp1);
			timecnt = (unsigned int)temp;
            timecnt1 = (unsigned int)temp1;
			newmsg = 0;
		}

		if (newprint)  { 
			P1OUT ^= 0x1; // Blink LED
// 			UART_printf("Hello %d\n\r test",timecnt); //  %d int, %ld long, %c char, %x hex form, %.3f float 3 decimal place, %s null terminated character array
			UART_send(2,(float)timecnt, (float)timecnt1);
